...
- 未ログイン利用者向けのはじめてのガイドとテストデータ
- アカウントごとの Event・ユーザー履歴・抽選結果
...
- `email_auth_users`: メールアドレスで登録したアカウントの認証情報
...
1. `.env.example` を参考に `.env` を作成し、`AUTH_REQUIRED=1` と `SESSION_SECRET` を設定します。
2. メールアドレス登録・ログインだけを利用する場合、X の設定は空欄のままで構いません。
3. X ログインも利用する場合、X Developer Console で OAuth 2.0 を有効にし、Callback URI に公開 URL の `/auth/callback` を正確に登録します。
4. `docker compose up -d --build` で起動します。

```dotenv
AUTH_REQUIRED=1
# メールログインのみなら、以下の X 設定は空欄で構いません。
...
access token は DB と Cookie に保存しません。`auth_users` には X ユーザー ID、username、表示名、画像 URL、ログイン日時だけを保存します。メールアドレス登録のパスワードは PBKDF2 による加塩ハッシュとして保存し、平文は保存しません。`AUTH_REQUIRED=0` のローカル起動ではログインなしで利用できます。

トップページはログインなしで直接開きます。

- 未ログイン: `単発抽選モード` になります。均等確率と特別条件を利用でき、抽選結果や参加回数は DB に保存されません。
- X またはメールアドレスでログイン後: `履歴保存モード` になります。Event、ユーザー履歴、抽選結果、同期履歴はアカウントごとに保存されます。

右上の `ログイン` から認証します。ログイン画面では X とメールアドレスを選べます。ログイン後は同じ主画面へ戻り、右上にアカウント名が表示されます。

メールアドレスログインには `.env` の `AUTH_REQUIRED=1` と `SESSION_SECRET` が必要です。X ログインも使う場合は、さらに `X_CLIENT_ID` と `X_REDIRECT_URI` を設定してから再構築してください。
