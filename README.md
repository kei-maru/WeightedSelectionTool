...
アプリはローカル Web 版（`web_app.py`）と Tkinter 版（`main.py`）があります。抽選セッション、応募レコード、当選結果、参加者履歴はローカル SQLite データベース `vrc_raffle.db` に保存されます。
...
起動するとブラウザで `http://127.0.0.1:8765/` が開きます。画面右上の「退出」ボタンでローカルサーバーを終了できます。
